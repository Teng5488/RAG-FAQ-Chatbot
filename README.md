...
自動從 **中文維基百科** 建立《我的英雄學院》英雄資料庫，並提供即時、可解釋的問答體驗。
...
-  **自動英雄知識庫建構**
  - 從中文維基百科擷取《我的英雄學院》角色資料
  - 依角色切分文本並注入上下文標籤（英雄檔案）
...
-  **本地大型語言模型（LLM）**
  - 透過 Ollama 運行
  - 支援模型：
    - `deepseek-r1:8b`（支援 `<think>` 思考鏈）
    - `llama3`

-  **思考鏈（Chain-of-Thought）可視化**
  - 自動解析 `<think>...</think>`
  - 於介面中獨立顯示「戰術分析過程」
...
├── faiss_index_hero_v3/ # FAISS 向量索引（首次執行自動建立）
├── assets/
│ └── My_Hero_Academia_logo.png
├── requirements.txt
└── README.md
